...
sub GET_DATA{
	...
	if(defined($opt->{onerow})){	
		...
		elsif($r && $opt->{tree_use}){ # Рекурсия (собираем всё дерево)
...
sub get_project_info{
	...
	unless($::params->{project}->{project_id}){
		print_error('Данный домен не найден');
		return ;
	}
	
### !!!! ОТСЮДА

	# Включение поддержки русских URL'ов	
...
# !!!! Сюда
	...
	if(!$::params->{TMPL_VARS}->{page_type}){
		...
		elsif($::params->{TMPL_VARS}->{const}->{PATH_INFO} eq '/robots.txt'){ # если запрашивался robots.txt -- выдаём дефалтовый
...
sub CONTEXT_SEARCH{
	...
	foreach my $part (@{$opt->{info}}){ # перебираем таблицы для выборки
		...
		unless($part->{table}){
			print_error("Не указан table! (CONTEXT_SEARCH)");
			return ;
		}
      ...
      my $str=param('capture_str');
			unless(&SQL_row("SELECT count(*) from capture WHERE project_id=? and str_key=? and str=?",undef,
				(
					...
					push @{$errors},
					"Проверочный числовой код введён не верно";
			}
		}
		foreach my $field (@{$form->{fields}}){
			...
			if($field->{regexp} && !($field->{value}=~m/$field->{regexp}/)){
				...
				if($field->{error_regexp}){
					...
					$err="Поле $field->{description} не заполнено или заполнено не верно";
				}
				push @{$errors},$err;
			}
		}
